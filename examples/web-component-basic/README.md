# `@syrotp/web-component` — basic example

Minimal Vite + plain-HTML demo of `<syrotp-verification>`.

```bash
pnpm --filter @syrotp/example-web-component-basic dev
```

Then open the URL Vite prints (default `http://localhost:5173`).

The page uses a hardcoded `verification` object so you can see the
...
then renders `<syrotp-verification verification="...">`.
